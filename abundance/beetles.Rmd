...
```{r}
bet_sorting %>% filter(siteID == "ORNL") %>% count(scientificName, sort=TRUE)

spec_site_dat <- bet_sorting %>% 
  filter(scientificName == "Cyclotrachelus fucatus", 
         siteID == "ORNL",
         collectDate == as.Date("2014-06-03")) %>% count(trapID)



%>% 
  mutate(trapID = paste(plotID, trapID, sep="_"),
    plot_int = as.numeric(as.factor(spec_site_dat$plotID))) %>% 
  arrange(trapID)
...
start_vec <- numeric(max(spec_site_dat$plot_int))  # numeric(10L) - we have 10 plots per site
